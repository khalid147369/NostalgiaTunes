          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border hover:bg-transparent">
                <TableHead className="pl-5 text-xs font-semibold uppercase tracking-wider text-muted-foreground">User</TableHead>
            ...
            <TableBody>
              {users.map((user) => (
                <TableRow key={user.id} className="border-border transition-colors hover:bg-secondary/40">
                  <TableCell className="py-3 pl-5">
                    <div className="flex items-center gap-3">
                      <Avatar className="size-9">
                        <AvatarFallback className="bg-primary/20 text-xs font-bold text-foreground">{user.initials}</AvatarFallback>
                      </Avatar>
                  ...
                  <TableCell>
                    <Badge variant="outline" className={`rounded-full px-2.5 ${roleStyles[user.role]}`}>
                      {user.role}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <span className="flex items-center gap-2 text-sm capitalize">
                      <span
                        className={user.status === 'active' ? 'size-2 rounded-full bg-accent' : 'size-2 rounded-full bg-destructive'}
                        aria-hidden="true"
                      />
                      {user.status}
                    </span>
                  ...
                  <TableCell className="pr-5 text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="glass rounded-xl border-glass-border">
                        <DropdownMenuItem className="rounded-lg">View profile</DropdownMenuItem>
                        <DropdownMenuItem className="rounded-lg">Change role</DropdownMenuItem>
